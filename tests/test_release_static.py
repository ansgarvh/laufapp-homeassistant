...
def test_versions_and_assets():
    ...
    assert 'APP_VERSION = "0.2.14"' in (ROOT/'laufapp/app/main_v0214.py').read_text()
    assert 'ARG BUILD_VERSION=0.2.14' in (ROOT/'laufapp/Dockerfile').read_text()
    assert 'main_v0214:app' in (ROOT/'laufapp/run.sh').read_text()
    assert '# Laufapp v0.2.14' in (ROOT/'README.md').read_text()
    assert '## v0.2.14 – 2026-08-31' in (ROOT/'CHANGELOG.md').read_text()
    assert (ROOT/'RELEASE_NOTES_v0.2.14.md').exists()
    assert 'Laufapp v0.2.14' in (ROOT/'RELEASE_NOTES_v0.2.14.md').read_text()
    static=ROOT/'laufapp/app/static'
    for name in ['index.html','styles.css','bugfix.css','app.js','manifest.webmanifest','sw.js','icon-192.png','icon-512.png','assets/bugfix.css','assets/v020.js','assets/v020.css','assets/v020_science.js','assets/v020_science.css','assets/v023_aggressiveness.js','assets/v025.js','assets/v025.css','assets/v0213.js','assets/v0213.css','assets/v0214.js','assets/v0214.css']:assert (static/name).exists()
    m=json.loads((static/'manifest.webmanifest').read_text());assert m['display']=='standalone'
    sw=(static/'sw.js').read_text()
    assert "const CACHE='laufapp-v0.2.14'" in sw
    assert 'assets/v0213.js?v=0.2.13' in sw and 'assets/v0213.css?v=0.2.13' in sw and 'assets/v0214.css?v=0.2.14' in sw and 'assets/v0214.js?v=0.2.14' in sw
    index=(static/'index.html').read_text()
    for asset in ['app.js?v=0.2.5','assets/bugfix.css?v=0.2.5','assets/v020.js?v=0.2.5','assets/v020_science.js?v=0.2.5','assets/v023_aggressiveness.js?v=0.2.5','assets/v025.js?v=0.2.5','assets/v025.css?v=0.2.5','assets/v0213.js?v=0.2.13','assets/v0213.css?v=0.2.13','assets/v0214.js?v=0.2.14','assets/v0214.css?v=0.2.14']:
    ...
    subprocess.run(['node','--check',str(static/'assets/v0214.js')],check=True)
...
def test_ha_app_config_and_health_auto_export_gateway():
    ...
    assert 'from main_v0214 import APP_VERSION' in gateway
    ...
    assert (ROOT/'laufapp/app/main_v0214.py').exists()
