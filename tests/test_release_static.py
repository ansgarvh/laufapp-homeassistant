...
import hashlib, json, struct, subprocess, yaml
...
def test_versions_and_assets():
    ...
    assert cfg['version']=='0.2.27'
    assert 'APP_VERSION = "0.2.27"' in (ROOT/'laufapp/app/main_v0227.py').read_text()
    assert 'ARG BUILD_VERSION=0.2.27' in (ROOT/'laufapp/Dockerfile').read_text()
    assert 'main_v0227:app' in (ROOT/'laufapp/run.sh').read_text()
    assert '# Laufapp v0.2.27' in (ROOT/'README.md').read_text()
    assert '## v0.2.27 – 2026-09-02' in (ROOT/'CHANGELOG.md').read_text()
    assert (ROOT/'RELEASE_NOTES_v0.2.27.md').exists()
    assert 'Laufapp v0.2.27' in (ROOT/'RELEASE_NOTES_v0.2.27.md').read_text()
    static=ROOT/'laufapp/app/static'
    for name in ['index.html','styles.css','bugfix.css','app.js','manifest.webmanifest','sw.js','icon.svg','icon-192.png','apple-touch-icon.png','assets/bugfix.css','assets/v020.js','assets/v020.css','assets/v020_science.js','assets/v020_science.css','assets/v023_aggressiveness.js','assets/v025.js','assets/v025.css','assets/v0213.js','assets/v0213.css','assets/v0214.js','assets/v0214.css','assets/v0215.css','assets/v0217.css','assets/v0220.css','assets/v0222.css','assets/v0223.css','assets/v0224.css','assets/v0225.js','assets/v0225.css','assets/v0227.css']:assert (static/name).exists()
    ...
    assert "const CACHE='laufapp-v0.2.27'" in sw
    ...
    assert 'app.js?v=0.2.27' in sw and 'assets/v0222.css?v=0.2.22' in sw and 'assets/v0223.css?v=0.2.23' in sw and 'assets/v0224.css?v=0.2.24' in sw
    assert 'assets/v0225.css?v=0.2.25' in sw and 'assets/v0225.js?v=0.2.25' in sw
    assert 'assets/v0227.css?v=0.2.27' in sw
    ...
    for asset in ['app.js?v=0.2.27','manifest.webmanifest?v=0.2.21','apple-touch-icon.png?v=0.2.21','assets/v0220.css?v=0.2.20','assets/v0222.css?v=0.2.22','assets/v0223.css?v=0.2.23','assets/v0224.css?v=0.2.24','assets/bugfix.css?v=0.2.5','assets/v020.css?v=0.2.18','assets/v020.js?v=0.2.18','assets/v020_science.js?v=0.2.5','assets/v023_aggressiveness.js?v=0.2.5','assets/v025.js?v=0.2.5','assets/v025.css?v=0.2.5','assets/v0213.js?v=0.2.13','assets/v0213.css?v=0.2.13','assets/v0215.css?v=0.2.15','assets/v0217.css?v=0.2.17']:
    ...
    entry0227=(ROOT/'laufapp/app/main_v0227.py').read_text()
    assert 'assets/v0227.css?v=0.2.27' in entry0227
    assert '_INDEX_V0227' in entry0227 and 'enrich_workout' in entry0227
...
def test_ha_app_config_and_health_auto_export_gateway():
    ...
    assert 'from main_v0227 import APP_VERSION' in gateway
    ...
    assert (ROOT/'laufapp/app/main_v0226.py').exists()
    assert (ROOT/'laufapp/app/main_v0227.py').exists()
    assert (ROOT/'laufapp/app/health_auto_export_v0226.py').exists()
    assert (ROOT/'laufapp/app/workout_phases_v0227.py').exists()
...
def test_github_repository_layout_and_security_audit():
    ...
    for required in ['pytest -q','python -m compileall','custom_components/laufapp_hae_relay','node --check','docker build','v023_aggressiveness.js','v025.js','v0225.js','assets/v0227.css?v=0.2.27','health-auto-export','home-assistant-relay','pip-audit','X-Forwarded-For','172.30.32.0/23','X-Remote-User-Id']:
