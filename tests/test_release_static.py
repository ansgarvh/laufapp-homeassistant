...
def test_versions_and_assets():
    ...
    assert all(label in races for label in ['A-Rennen','B-Rennen','C-Rennen','api/v2/races','Vergangene Trainingstage'])
    ...
    calendar=(ROOT/'laufapp/app/training_v020.py').read_text()
    assert all(x in calendar for x in ['post_a_recovery','post_a_reentry','replan_existing_future_weeks','support_races_for_week','past_protected'])
    planner=(ROOT/'laufapp/app/training_planner_v020.py').read_text()
    assert 'Race week must produce the actual A-race for every supported distance' in planner
    assert (ROOT/'tests/test_v0218_race_calendar.py').exists()
