...
def test_versions_and_assets():
    ...
    assert cfg['version']=='0.2.19'
    assert 'APP_VERSION = "0.2.19"' in (ROOT/'laufapp/app/main_v0219.py').read_text()
    assert 'ARG BUILD_VERSION=0.2.19' in (ROOT/'laufapp/Dockerfile').read_text()
    assert 'main_v0219:app' in (ROOT/'laufapp/run.sh').read_text()
    assert '# Laufapp v0.2.19' in (ROOT/'README.md').read_text()
    assert '## v0.2.19 – 2026-09-01' in (ROOT/'CHANGELOG.md').read_text()
    assert (ROOT/'RELEASE_NOTES_v0.2.19.md').exists()
    assert 'Laufapp v0.2.19' in (ROOT/'RELEASE_NOTES_v0.2.19.md').read_text()
    ...
    assert "const CACHE='laufapp-v0.2.19'" in sw
...
def test_ha_app_config_and_health_auto_export_gateway():
    ...
    assert 'from main_v0219 import APP_VERSION' in gateway
