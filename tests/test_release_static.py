...
def test_versions_and_assets():
    ...
    assert cfg['version']=='0.1.5'
    assert 'APP_VERSION = "0.1.5"' in (ROOT/'laufapp/app/db.py').read_text()
    assert 'ARG BUILD_VERSION=0.1.5' in (ROOT/'laufapp/Dockerfile').read_text()
    ...
    assert "const CACHE='laufapp-v0.1.5'" in (static/'sw.js').read_text()
