...
def test_home_assistant_relay_templates_do_not_embed_secrets():
    ...
    assert "hooks.nabu.casa" not in rest
    assert STRONG_TOKEN not in rest
    assert "REPLACE_WITH_A_NEW_RANDOM_WEBHOOK_ID" in automation
    assert "local_only: true" in automation
    assert "mode: queued" in automation and "max: 50" in automation
