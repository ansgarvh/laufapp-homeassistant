...
def _race(c,name,days,priority='A',distance=42.195,goal=12000):
    ...
    mapping=dict(set_setting.__globals__['get_setting'](c,'race_priorities',{}) or {})
...
def test_multiple_a_races_switch_focus_after_first(tmp_path):
    ...
    try:
        ...
        second,d2=_race(c,'Herbst-A',140,'A')
...
def test_b_race_replaces_longrun_only_in_its_week(tmp_path):
    c=_db(tmp_path)
    try:
        _,a_date=_race(c,'A-Marathon',70,'A')
        ...
        set_setting(c,'race_priorities',{str(int(cur.lastrowid)):'B'} | {str(int(c.execute("SELECT id FROM races WHERE name='A-Marathon'").fetchone()['id'])):'A'})
        c.commit()
...
def test_two_consecutive_specific_load_weeks_progress(tmp_path):
    c=_db(tmp_path)
    try:
        # Sunday makes the Monday week-start differences deterministic.
        ...
        set_setting(c,'race_priorities',{str(int(cur.lastrowid)):'A'});set_setting(c,'training_volume_profile','steady');c.commit()
        r=c.execute("SELECT * FROM races WHERE id=?",(int(cur.lastrowid),)).fetchone()
