...
def _cleanup_generated_duplicates(c,key:str):
    """Repair v0.1.8 duplicate engine rows without touching user/completed history."""
    rows=c.execute("SELECT id,scheduled_date,status,linked_run_id,COALESCE(manual_override,0) manual_override,COALESCE(modified_by,'') modified_by FROM workouts WHERE origin_week_start=? ORDER BY id DESC",(key,)).fetchall()
    protected_dates={r['scheduled_date'] for r in rows if r['status']!='planned' or r['linked_run_id'] is not None or r['manual_override'] or r['modified_by'] not in {'','engine'}}
    seen=set()
    for r in rows:
        untouched=r['status']=='planned' and r['linked_run_id'] is None and not r['manual_override'] and r['modified_by'] in {'','engine'}
        if not untouched:continue
        day=r['scheduled_date']
        if day in protected_dates or day in seen:c.execute("DELETE FROM workouts WHERE id=?",(r['id'],))
        else:seen.add(day)

def _templates_after_protected(templates,protected):
    remaining=list(templates)
    for r in protected:
        typ=r['workout_type'];idx=next((i for i,t in enumerate(remaining) if t[0]==typ),None)
        if idx is None and remaining:idx=0
        if idx is not None:remaining.pop(idx)
    return remaining

def _fit_templates_to_slots(templates,slots:int):
    """Keep the most important session roles if only part of a current week remains."""
    if slots<=0:return []
    if len(templates)<=slots:return list(templates)
    indexed=list(enumerate(templates))
    priority={'race':0,'long':1,'quality':2,'raceprep':2,'easy':3}
    chosen=sorted(indexed,key=lambda x:(priority.get(x[1][0],4),x[0]))[:slots]
    return [t for _,t in sorted(chosen,key=lambda x:x[0])]

def generate_week(c,ws:date|None=None,force=False):
    ws=week_start_for(ws or date.today());key=ws.isoformat();_cleanup_generated_duplicates(c,key)
    ...
    protected=c.execute("SELECT * FROM workouts WHERE origin_week_start=? ORDER BY scheduled_date,id",(key,)).fetchall()
    ...
    week_end=ws+timedelta(days=6);week_rows=c.execute("SELECT * FROM workouts WHERE scheduled_date BETWEEN ? AND ? ORDER BY scheduled_date,id",(key,week_end.isoformat())).fetchall();occupied={r['scheduled_date'] for r in week_rows}
    remaining_templates=_templates_after_protected(templates,protected)
    available_dates=[d for d in dates if d>=date.today() and d.isoformat() not in occupied]
    remaining_templates=_fit_templates_to_slots(remaining_templates,len(available_dates))
    protected_km=sum(float(r['distance_km'] or 0) for r in week_rows);budget=max(0.0,total-protected_km)
    if remaining_templates and budget>0:
        requested=sum(float(t[2]) for t in remaining_templates);scale=min(1.0,budget/requested) if requested>0 else 0.0
    else:scale=0.0
    generation=datetime.now(timezone.utc).isoformat()
    for scheduled,t in zip(available_dates,remaining_templates):
        typ,title,km,zone,rpe,purpose,instructions=t;km=float(km)*scale
        if km<1.0:continue
...
def refresh_plan(c,start:date|None=None,weeks=4):
    ...
    for i in range(weeks):
        ws=start+timedelta(days=7*i);_cleanup_generated_duplicates(c,ws.isoformat());rows0=[_wdict(r) for r in c.execute("SELECT * FROM workouts WHERE week_start=? ORDER BY scheduled_date,id",(ws.isoformat(),))]
        if i==0:old=rows0
