...
class LongRunPlanner:
    def plan(self,c,race,ws:date,phase:TrainingPhase,total_km:float,readiness:RecoveryState)->LongRunDecision:
        ...
        # Race week must produce the actual A-race for every supported distance,
        # not only for marathons. The previous ordering accidentally sent 5k/10k/
        # half-marathon A-races through the ordinary Long Run branch.
        if phase is TrainingPhase.RACE:
            duration=max(1.0,float(race["goal_seconds"])/60);goal_pace=float(race["goal_seconds"])/max(dist,.1)
            if dist<=10:high,moderate=duration*.72,duration*.18
            elif dist<=25:high,moderate=duration*.25,duration*.60
            else:high,moderate=duration*.10,duration*.70
            low=max(0,duration-high-moderate);load=TrainingLoad(dist,duration,"race",low,moderate,high,moderate+high,moderate,high,0,0,0,10,_load_score(low,moderate,high,duration,10,True))
            title=f"WETTKAMPF · {race['name']}";session=PlannedSession("race",title,dist,"a_race","Wettkampf","Zielwettkampf","Kontrolliert eröffnen und Renntaktik/Fueling umsetzen.",PhysiologicalTarget.RACE,"race_a",WorkoutType.RACE.value,load,"Das A-Rennen ist der zentrale Belastungsreiz der Woche und ersetzt den Longrun.",{"mp_km":dist if dist>=40 else 0,"goal_pace_s_per_km":round(goal_pace,1)})
            return LongRunDecision(session,"race",previous_distance,previous_mp)
        if dist<40:
