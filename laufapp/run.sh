...
uvicorn main_v0215:app \
  --host 0.0.0.0 --port 8099 \
  --no-proxy-headers --no-server-header \
  --limit-concurrency 64 --timeout-keep-alive 5 &
MAIN_PID=$!
echo "LAUFAPP_PROCESS_STARTED child=main pid=$MAIN_PID port=8099 version=0.2.15" >&2
...
then
  ...
  GATEWAY_PID=$!
  echo "LAUFAPP_PROCESS_STARTED child=gateway pid=$GATEWAY_PID port=8100 version=0.2.15" >&2
