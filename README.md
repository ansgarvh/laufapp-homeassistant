...
## Neu in v0.2.21 – Header-Icon-Auslieferung repariert

- Das in v0.2.20 freigegebene Lauf-Icon bleibt unverändert; behoben wurde ausschließlich dessen HTTP-Auslieferung im Header.
- Die Browserpfade `/icon-192.png` und `/icon-512.png` werden jetzt explizit als `image/png` bereitgestellt. Zuvor führte der direkte Header-Aufruf von `/icon-192.png` zu HTTP 404 und Safari zeigte einen Bildfehler-Platzhalter.
- Der Header lädt `icon-192.png?v=0.2.21`, damit ein zuvor fehlgeschlagener Abruf nicht aus einem Browser-/WebView-Cache übernommen wird.
- Neue Regressionstests rufen exakt die Browser-URL auf, verlangen HTTP 200 und validieren die PNG-Struktur inklusive CRC und IDAT-Dekompression.
- Keine Datenbankschemamigration und keine Änderung an Trainingsengine, Apple Health, HAE/Nabu/Ingress, Security oder dem unabhängig versionierten Home-Assistant-Relay.
...
Benutzerdaten liegen im persistenten Home-Assistant-`/data`-Bereich. v0.2.21 benötigt **keine Datenbankschemamigration**.
...
Vor Merge laufen Python-Compilecheck einschließlich Custom Integration, JavaScript-Syntaxchecks, vollständige Pytest-Regression über den v0.2.21-Entry-Point, realitätsnahe HAE-v2-Regressionstests, explizite HTTP- und PNG-Dekodierungsregression für die Brand-Icons, >262144-Zeichen-Relaytest, Rate-/Slow-Body-Webhooktests, 16-Wochen-Marathonsimulation, neun randomisierte Läuferprofile, `pip check`, `pip-audit`, Git-History-Secret-Scan, Bandit-Gate, Docker-Build, direkter HAE-E2E, interner Relay-E2E, externe Ingress-Spoofing-Negativtests und positive Home-Assistant-Ingress-Netzsimulation.
...
uvicorn main_v0221:app --host 127.0.0.1 --port 8099 --no-proxy-headers
```

Weitere Details: `SECURITY.md`, `NABU_CASA_HEALTH_SYNC.md`, `RELEASE_NOTES_v0.2.21.md`, `TRAINING_ENGINE.md`, `MIGRATIONS.md`.